...
int jogadas = 0; // Total de jogadas feitas.
const int n; // n é o número de discos que o usuário escolher
char final; // Caracter de controle. Feito pra pular pro final ou não.
...
if(quant % 2==0){
    if(aux->topo == NULL || mae->topo->peso < aux->topo->peso){
        ...
        if(final == 'f')sleep(1);
        if(mae->topo != NULL && *flag != 0 && mae->topo->peso > *flag)return 0;// Verifico se já cheguei na flag. Se sim, paro de esvaziar essa torre
